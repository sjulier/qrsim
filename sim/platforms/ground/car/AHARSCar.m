classdef AHARSCar<AHARS
    % Simulates the attitude-heading-altitude reference system present on the car.
    ...
    %
    % AHARSCar Methods:
    %   AHARSCar(objparams) - constructs the object
    ...
    methods (Sealed)
       function obj = AHARSCar(objparams)
            % constructs the object
            %
            % Example:
            %
            %   obj = AHARSCar(objparams)
            ...
            assert(isfield(objparams,'accelerometer')&&isfield(objparams.accelerometer,'on'),'ahahrscar:noaccelerometer',...
                'the platform config file must define an accelrometer if not needed set accelrometer.on = 0');
            
            if(objparams.accelerometer.on)
                assert(isfield(objparams.accelerometer,'type'),'ahahrscar:noaccelerometertype',...
            ...
            assert(isfield(objparams,'gyroscope')&&isfield(objparams.gyroscope,'on'),'ahahrscar:nogyroscope',...
                'the platform config file must define an gyroscope if not needed set gyroscope.on = 0');
            
            if(objparams.gyroscope.on)
                assert(isfield(objparams.gyroscope,'type'),'ahahrscar:nogyroscopetype',...
            ...
            assert(isfield(objparams,'altimeter')&&isfield(objparams.altimeter,'on'),'ahahrscar:noaltimeter',...
                'the platform config file must define an altimeter if not needed set altimeter.on = 0');
            
            if(objparams.altimeter.on)
                assert(isfield(objparams.altimeter,'type'),'ahahrscar:noaltimetertype',...
            ...
            assert(isfield(objparams,'orientationEstimator')&&isfield(objparams.orientationEstimator,'on'),'ahahrscar:noorientationestimator',...
            ...
            if(objparams.orientationEstimator.on)
                assert(isfield(objparams.orientationEstimator,'type'),'ahahrscar:noorientationestimatortype',...
        ...
        function estimatedAHA = getMeasurement(obj,stateAndAccelerations)
            ...
            %        X   - platform noise free state vector [px,py,pz,phi,theta,psi]
        ...
        function obj = setState(obj,X)
            ...
            %
            %   obj.setState(X)
            %       X - platform new state vector [px,py,pz,phi,theta,psi]
