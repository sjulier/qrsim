classdef BoxWithHousesAndPersonsArea<Area
    % Defines a simple box shaped area in which the ground is split in
    % areas of different classes, there are persons present at randomly
    % generated locations and house like box objects at prespecified positions.
    %
    ...
    methods (Sealed,Access=public)
        function obj = BoxWithHousesAndPersonsArea(objparams)
            % constructs the object
            %
            % Example:
            %
            %   obj=BoxWithHousesAndPersonsArea(objparams)
            ...
            assert(isfield(objparams,'numpersonsrange'),'boxwithhousesandpersonsarea:nonumpersonsrange',...
                'If using a BoxWithHousesAndPersonsArea, the task must define the parameter numpersonsrange');
            obj.numPersonsRange = objparams.numpersonsrange;
            
            assert(isfield(objparams,'personsize'),'boxwithhousesandpersonsarea:nopersonssize',...
                'If using a BoxWithHousesAndPersonsArea, the task must define the parameter personssize');
            obj.personSize = objparams.personsize;
            
            assert(isfield(objparams,'personfounddistancethreshold'),'boxwithhousesandpersonsarea:nopersonfounddistancethreshold',...
                'If using a BoxWithHousesAndPersonsArea, the task must define the parameter personfounddistancethreshold');
            obj.dthr = objparams.personfounddistancethreshold;
            
            assert(isfield(objparams,'personfoundspeedthreshold'),'boxwithhousesandpersonsarea:nopersonfoundspeedthreshold',...
                'If using a BoxWithHousesAndPersonsArea, the task must define the parameter personfoundspeedthreshold');
            obj.sthr = objparams.personfoundspeedthreshold;
            
            assert(isfield(objparams,'terrain') && isfield(objparams.terrain,'type'),'boxwithhousesandpersonsarea:noterraintype',...
                'If using a BoxWithHousesAndPersonsArea, the task must define the parameter terrain.type');
                                    
            assert(isfield(objparams,'personinclassprob'),'boxwithhousesandpersonsarea:nopersoninclassprob',...
                'If using a terrain of type PourTerrain, the config file must define the array personincalssprob'); 
            
            assert(sum(objparams.personinclassprob)<=1,'boxwithhousesandpersonsarea:badpersoninclassprob',...
            ...
            assert(isfield(objparams,'boxes'),'boxwithhousesandpersonsarea:noboxes','for this type of flight area the task must define obstacles environment.area.boxes');
            tmp.boxes = objparams.boxes;
